...
auth: state,
browser: ['The-Bodysuit-Bot','Edge','1.0.0'],
}
...
await conn.sendHydrated(`5212411719888@s.whatsapp.net`, `┌「𝚃𝙷𝙴-BODYSUIT -𝙱𝙾𝚃-𝙼𝙳」\n├❏𝙷𝙾𝙻𝙰 ALEX , 𝚂𝙴 𝙳𝙴𝚃𝙴𝙲𝚃𝙾 𝚀𝚄𝙴 𝙴𝚂𝚃𝙴 𝙽𝚄𝙼𝙴𝚁𝙾 𝙴𝚂 𝚄𝙽 𝙽𝚄𝙴𝚅𝙾 𝙱𝙾𝚃 𝙰𝙲𝚃𝙸𝚅𝙾 ✅\n└────ׂ─ׂ─ׂ─ׂ───`, author, pp, null, null, null, null, [['💎𝙾𝚆𝙽𝙴𝚁🎟️', '/owner'], ['🕋𝙼𝙴𝙽𝚄 𝙿𝚁𝙸𝙽𝙲𝙸𝙿𝙰𝙻💎', '#menu']])
await conn.groupAcceptInvite('Lf7uQcrPFiJGvmW6xAQ6SN')}}

process.on('uncaughtException', console.error)

let isInit = true;
let handler = await import('./sistema de control.js')
global.reloadHandler = async function (restatConn) {
try {
const Handler = await import(`./sistema de control.js?update=${Date.now()}`).catch(console.error)
...
const pluginFolder = global.__dirname(join(__dirname, './bodysuit/index'))
...
conn.logger.warn(`deleted bodysuit- '${filename}'`)
return delete global.plugins[filename]
}
} else conn.logger.info(`new bodysuit- '${filename}'`)
...
conn.logger.error(`error require bodysuit'${filename}\n${format(e)}'`)
} finally {
